...
const TaskContent: React.FC<{
  ...
  useEffect(() => {
    if (myTaskTree.children.length > 0) {
      ...
      // If all subtasks are checked, mark the task as checked
      // If there are any unchecked subtasks, uncheck the task
  ...
  // Save text when editing is finished
  const handleBlur = () => {
      ...
      {myTaskTree.taskId === rootId ? (
        ...
        >
          ...
          <div>
            <div />
            {/* Check icon 
                Cannot be clicked if there are subtasks
            */}
            <IconButton
              style={{
          ...
          <div
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                e.preventDefault();
                handleBlur();  // Confirm edit
                editableRef.current?.blur(); // Remove focus and exit edit mode
              }
            }}
            style={{ cursor: "text" }}
          >
          ...
          <DeleteForeverIcon
            ...
            onClick={() => {
              const shouldDelete = window.confirm(
                myTaskTree.label + "Do you want to delete ?"
              );
              if (shouldDelete) {
                handleDelete();
